...
public:
    ...
    explicit FunctionArrayElement(ContextPtr context_) : context(context_) { }
...
private:
    ...
    template <typename DataType>
    static ColumnPtr
    executeArrayNumberConst(const ColumnsWithTypeAndName & arguments, const Field & index, ArrayImpl::NullMapBuilder & builder);
    ...
    /** For a map array, the function is evaluated component-wise for its key and value
      */
...
FunctionPtr FunctionArrayElement::create(ContextPtr context_)
{
    return std::make_shared<FunctionArrayElement>(context_);
}
    ...
    const ColumnsWithTypeAndName & arguments, const Field & index, ArrayImpl::NullMapBuilder & builder)
{
    ...
    const ColumnVector<DataType> * col_nested_elem = nullptr;
    const auto * col_nullable = checkAndGetColumn<ColumnNullable>(col_nested_array->getDataPtr().get());
    if (!col_nullable)
        col_nested_elem = checkAndGetColumn<ColumnVector<DataType>>(&col_nested_array->getData());
    else
        col_nested_elem = checkAndGetColumn<ColumnVector<DataType>>(col_nullable->getNestedColumnPtr().get());

    if (!col_nested_elem)
        return nullptr;

    const auto * null_map = col_nullable ? &col_nullable->getNullMapColumn() : nullptr;
    auto res_data = ColumnVector<DataType>::create();
    auto res_offsets = ColumnArray::ColumnOffsets::create();
    auto res_null_map = col_nullable ? ColumnUInt8::create() : nullptr;
    ...
    if (col_nullable)
        return ColumnArray::create(ColumnNullable::create(std::move(res_data), std::move(res_null_map)), std::move(res_offsets));
    else
        return ColumnArray::create(std::move(res_data), std::move(res_offsets));
}
    ...
    const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, ArrayImpl::NullMapBuilder & builder, size_t input_rows_count) const
{
    ...
    if (!((res = executeNumber<IndexType, UInt8>(arguments, index_data, builder))
          ...
          || (res = executeArrayNumber<IndexType, UInt8>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, UInt16>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, UInt32>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, UInt64>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, Int8>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, Int16>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, Int32>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, Int64>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, Float32>(arguments, index_data, builder))
          || (res = executeArrayNumber<IndexType, Float64>(arguments, index_data, builder))
...
{
    if (!context->getSettingsRef().precise_float_parsing)
        return nullptr;
...
ColumnPtr FunctionArrayElement::perform(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type,
                                   ArrayImpl::NullMapBuilder & builder, size_t input_rows_count) const
{
    // std::cout << "perform:" << Block(arguments).dumpStructure() << ",,,,,,,result_type:" << result_type->getName() << std::endl;
    ColumnPtr res;
    ...
    else
    {
        ...
        if (!((res = executeNumberConst<UInt8>(arguments, index, builder)) || (res = executeNumberConst<UInt16>(arguments, index, builder))
              || (res = executeNumberConst<UInt32>(arguments, index, builder))
              || (res = executeNumberConst<UInt64>(arguments, index, builder))
              || (res = executeNumberConst<Int8>(arguments, index, builder)) || (res = executeNumberConst<Int16>(arguments, index, builder))
              ...
              || (res = executeArrayNumberConst<UInt8>(arguments, index, builder))
              || (res = executeArrayNumberConst<UInt16>(arguments, index, builder))
              || (res = executeArrayNumberConst<UInt32>(arguments, index, builder))
              || (res = executeArrayNumberConst<UInt64>(arguments, index, builder))
              || (res = executeArrayNumberConst<Int8>(arguments, index, builder))
              || (res = executeArrayNumberConst<Int16>(arguments, index, builder))
              || (res = executeArrayNumberConst<Int32>(arguments, index, builder))
              || (res = executeArrayNumberConst<Int64>(arguments, index, builder))
              || (res = executeArrayNumberConst<Float32>(arguments, index, builder))
              || (res = executeArrayNumberConst<Float64>(arguments, index, builder))
              || (res = executeStringConst(arguments, index, builder)) || (res = executeArrayStringConst(arguments, index, builder))
