...
#cp secret pem(key file) to driver host. ignore "Permission denied" error
echo "$(date '+%F %T'): cp secret pem(key file) to driver host"
...
#copy emr id and ip files to driver host for timely cluster shutting down control
scp -i ${local_key_file}  /tmp/emr_cluster_id ${cloud_vm_user}@${driver_host}:/tmp/
scp -i ${local_key_file}  /tmp/*_ip ${cloud_vm_user}@${driver_host}:/tmp/
...
if [ $? -ne 0 ];then
        echo "$(date '+%F %T'): sqls upload to driver failed!"
        exit 1
fi
...
do
	...
	ssh -o StrictHostKeyChecking=no -i ${local_key_file} ${cloud_vm_user}@${driver_host}  << EOF
	cd ${script_home}
...
do
  ...
  now_time=$(date "+%Y_%m_%d_%H_%M_%S")
	...
	echo "${result_home}/${sv}_${now_time}" > /tmp/result_dir
...
ksh ${script_home}/uploadResultToConbench.sh
